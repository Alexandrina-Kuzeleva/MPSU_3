...
public static class DataContext
{
    ...
    public static int NextId<T>() where T : class
    {
        ...
        {
            var t when t == typeof(Room) => 
                Rooms.Count == 0 ? 1 : Rooms.Max(x => x.Id) + 1,
            var t when t == typeof(Teacher) => 
                Teachers.Count == 0 ? 1 : Teachers.Max(x => x.Id) + 1,
            var t when t == typeof(Group) => 
                Groups.Count == 0 ? 1 : Groups.Max(x => x.Id) + 1,
            var t when t == typeof(Course) => 
                Courses.Count == 0 ? 1 : Courses.Max(x => x.Id) + 1,
            var t when t == typeof(Session) => 
                Sessions.Count == 0 ? 1 : Sessions.Max(x => x.Id) + 1,
            _ => throw new NotSupportedException(
                $"No ID generator for {typeof(T)}"
            )
        };
    }
