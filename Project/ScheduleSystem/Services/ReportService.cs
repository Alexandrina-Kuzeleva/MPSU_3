...
public static class ReportService
{
    ...
    public static void PrintGroupReport(int groupId, DateOnly? from = null, DateOnly? to = null, string format = "text")
    {
        ...
        if (format == "csv") ExportToCsv(sessions, $"report_group_{groupId}.csv");
        else if (format == "json") ExportToJson(sessions, $"report_group_{groupId}.json");
        else TablePrinter.PrintSessions(sessions);
    }

    public static void PrintTeacherReport(int teacherId, DateOnly? from = null, DateOnly? to = null, string format = "text")
    {
        var sessions = FilterSessions(s => s.TeacherId == teacherId, from, to);
        var teacherName = DataContext.Teachers.FirstOrDefault(t => t.Id == teacherId)?.Name ?? $"Teacher {teacherId}";

        PrintReportHeader($"Schedule for teacher {teacherName}", from, to, sessions);

        if (format == "csv") ExportToCsv(sessions, $"report_teacher_{teacherId}.csv");
        else if (format == "json") ExportToJson(sessions, $"report_teacher_{teacherId}.json");
        else TablePrinter.PrintSessions(sessions);
    }

    public static void PrintRoomReport(int roomId, DateOnly? from = null, DateOnly? to = null, string format = "text")
    {
        var sessions = FilterSessions(s => s.RoomId == roomId, from, to);
        var roomCode = DataContext.Rooms.FirstOrDefault(r => r.Id == roomId)?.Code ?? $"Room {roomId}";

        PrintReportHeader($"Schedule for room {roomCode}", from, to, sessions);

        if (format == "csv") ExportToCsv(sessions, $"report_room_{roomId}.csv");
        else if (format == "json") ExportToJson(sessions, $"report_room_{roomId}.json");
        else TablePrinter.PrintSessions(sessions);
    }
    ...
    public static void PrintDayReport(DateOnly date, string format = "text")
    {
        ...
        PrintReportHeader($"Schedule for {date:yyyy-MM-dd} ({date.DayOfWeek})", null, null, sessions);

        if (format == "csv") ExportToCsv(sessions, $"report_day_{date:yyyy-MM-dd}.csv");
        else if (format == "json") ExportToJson(sessions, $"report_day_{date:yyyy-MM-dd}.json");
        else TablePrinter.PrintSessions(sessions);
    }

    private static void PrintReportHeader(string title, DateOnly? from, DateOnly? to, List<Session> sessions)
    {
        if (!sessions.Any())
        {
    ...
    private static void ExportToCsv(List<Session> sessions, string filename)
    {
        ...
        foreach (var s in sessions)
        {
            ...
            lines.Add($"{s.Date:yyyy-MM-dd},{s.DayShort},{s.TimeRange},{course},{teacher},{group},{room},{s.Notes.Replace(",", " ")}");
        }

        File.WriteAllLines(filename, lines);
        Console.WriteLine($"Exported {sessions.Count} rows → {filename}");
    }
