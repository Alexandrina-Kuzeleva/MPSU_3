...
def index(request):
    ...
    actions = [
        ("/secure/delete_user/2/", "Удалить пользователя (уязвимо: без серверной проверки роли)"),
        ("/secure/admin_panel/", "Админ-панель (только клиентская проверка)"),
        ("/secure/promote_user/2/", "Повышение роли (уязвимо: неверный RBAC — доверяет параметру role)"),
    ]
